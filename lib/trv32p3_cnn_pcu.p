/*
-- File : trv32p3_cnn_pcu.p
--
-- Contents : Definition of the trv32p3_cnn controller in PDG.
--
...
void trv32p3_cnn::user_next_pc() {
  ...
  bool br_EX   = jump_of13_cd_sig; // only set if branch is taken
  bool jalr_ID = jump_trgt_sig;
  bool jal_ID  = jump_of21_sig;

  // at most one source of change:
  PDG_ASSERT( ((uint8_t)br_EX + (jal_ID && !br_EX) + (jalr_ID && !br_EX)) <= 1 );

  DEBUG_PRINT_PCU("jump %d%d%d",br_EX,jalr_ID,jal_ID);
  ...
  else if (jalr_ID) jump_pc = jmp_tgt_ID;
  else if (jal_ID)  jump_pc = jmp_tgt_ID;

  bool jump = br_EX || jal_ID || jalr_ID;
  ...
  bool could_jump = br_EX || (jalr_ID && !hw_stall_sig()) || (jal_ID && !hw_stall_sig());
