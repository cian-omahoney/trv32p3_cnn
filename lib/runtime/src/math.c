...
float expf(float x)     /* default IEEE float exp */
{
    ...
    // suppress warnings about uninitialized variables - not used in some code paths
    hi = chess_dont_care(float);
    lo = chess_dont_care(float);
...
double exp(double x)    /* default IEEE double exp */
{
    ...
    // suppress warnings about uninitialized variables - not used in some code paths
    hi = chess_dont_care(double);
    lo = chess_dont_care(double);
...
float asinf(float x)
{
      ...
      qS4 =  7.7038154006e-02; /* 0x3d9dc62e */
        ...
        float t,w,p,q,c,r,s;
        int32_t hx,ix;
        t = chess_dont_care(float); // suppress 'uninitialized' warning
        GET_FLOAT_WORD(hx,x);
        ix = hx&0x7fffffff;
        if(ix==0x3f800000) {
