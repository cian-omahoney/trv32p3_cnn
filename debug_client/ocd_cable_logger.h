...
public:
  ...
  virtual ~ocd_cable_logger() = default;
    ...
    int add_iwrite (long long v) override {
      enter ("add_iwrite",
             pdc_opcode (v) + " (" + fmt(VBit<16, false>(v)) + ")");
      auto r = Base::add_iwrite (v);
      leave ("add_iwrite",
             pdc_opcode (v) + " (" + fmt(VBit<16, false>(v)) + ")", r);
      return r;
    }

    int add_dwrite (ocd_cable::reg_type r, const DataType &v) override {
      enter ("add_dwrite", to_string (r), fmt(v));
      auto rc = Base::add_dwrite (r, v);
      leave ("add_dwrite", to_string (r), fmt(v), rc);
      return rc;
    }
    ...
    int add_dwrite_dread (ocd_cable::reg_type r, const DataType &v) override {
      enter ("add_dwrite_dread", to_string (r), fmt(v));
      auto rc = Base::add_dwrite_dread (r, v);
      leave ("add_dwrite_dread", to_string (r), fmt(v), rc);
      return rc;
    }
...
private:
  ...
  static inline std::string fmt(VBitWrapper const& v) {
    unsigned const sz = v.getBitSize();
    if (sz > 0) {
      if (0 == (sz % 4)) return "0x" + v.toHex();
      if (0 == (sz % 3)) return "0o" + v.toOct();
      return "0b" + v.toBin();
    }
    return "(invalid)";
  }
