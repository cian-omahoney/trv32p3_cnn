...
Checkers_pdc_interface::~Checkers_pdc_interface()
{
  try {
    ...
    uicore()->errors.put(chkapi_ERR, e.what(), "flush_write_merge_buffers");
  }
}
...
void Checkers_pdc_interface::reset_pdc()
{
 // disable hardware breakpoints/watchpoints
    ...
    try {
      ...
      uicore()->errors.put(chkapi_ERR, e.what(), "disable_hw_break");
      return;
    }
    ...
    for (int i = 0; i < pdc_commands::nr_breakpoints; i++)
        if (hw_break_in_use[i] == -1) {  // set new breakpoint
            ...
            try {
              ...
              uicore()->errors.put(chkapi_ERR, e.what(), "enable_hw_break");
              return -1;
            }
    ...
    if (p != cache.end()) {
        ...
        if (merge_line.covered(ar,er)) {
            ...
            try {
              ...
              uicore()->errors.put(chkapi_ERR, e.what(), "flush");
              return failed;
            }
        }
