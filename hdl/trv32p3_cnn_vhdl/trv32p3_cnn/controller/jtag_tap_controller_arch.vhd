...
architecture rtl of jtag_tap_controller is
  ...
  signal jtag_ireg_master : std_logic_vector(15 downto 0);
  signal jtag_ireg_sig : std_logic_vector(15 downto 0);
  signal jtag_ireg_slave : std_logic_vector(15 downto 0);
...
begin
  ...
  jtag_ireg_master_input : process(state, jtag_ireg_master, jtag_tdi_in)
  begin
    jtag_ireg_sig <= (others => '0');
    jtag_tdo_sig <= '0';
  ...
  end process jtag_ireg_master_input;

  clock_jtag_ireg_master : process(jtag_tck_in, jtag_trst)
  begin
    if jtag_trst /= '1' then
      jtag_ireg_master <= (others => '0');
    elsif jtag_tck_in'event and jtag_tck_in = '1' then
      if state = CAPTURE_IR_STATE or state = SHIFT_IR_STATE then
        jtag_ireg_master <= jtag_ireg_sig;
      end if;
    end if;
  end process clock_jtag_ireg_master;

  clock_jtag_ireg_slave : process(jtag_tck_in, jtag_trst)
  begin
    if jtag_trst /= '1' then
  ...
  end process clock_jtag_ireg_slave;
  ...
  begin
    if jtag_trst /= '1' then
      ...
      if jtag_ireg_slave = JTAG_BYPASS_INSTR then
        if state = CAPTURE_DR_STATE then
          jtag_tdi_REG <= '0';
    ...
    if jtag_trst /= '1' then
      ...
      if state = SHIFT_DR_STATE then
        if jtag_ireg_slave = JTAG_BYPASS_INSTR then
          ...
          if jtag_ireg_slave(15 downto 11) = "00001" then
            jtag_tdo_REG <= jtag_tap_si_in;
          else
            jtag_tdo_REG <= '0';
          end if;
        end if;
