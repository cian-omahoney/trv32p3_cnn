...
begin
  ...
  sync_update_ir_proc : process(clock, reset)
  begin
    if reset /= '0' then
  ...
  -- synchronisation to processor clock
  sync_update_dr_proc : process(clock, reset)
  begin
    if reset /= '0' then
  ...
  align_to_lsb : process(dbg_instr_in,
                         dbg_data_pi)
  begin
    dbg_data_pi_out <= (others => '0');
    case dbg_instr_in(1 downto 0) is
      when "00" => -- DBG_DATA_REG_INDX
        dbg_data_pi_out(7 downto 0) <= dbg_data_pi(31 downto 24);
      when "01" => -- DBG_ADDR_REG_INDX
        dbg_data_pi_out <= dbg_data_pi;
      when "10" => -- DBG_INSTR_REG_INDX
        dbg_data_pi_out <= dbg_data_pi;
      when others =>
        dbg_data_pi_out <= dbg_data_pi;
    end case;
  end process align_to_lsb;
