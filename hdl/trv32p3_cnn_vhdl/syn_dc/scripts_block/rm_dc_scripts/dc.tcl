...
# Changed to:
    if {[info exists env(ASIP_SAIF_FILE)]} {
      # For SAIF files generated from VHDL we have to ignore case during read:
      set ORG_find_ignore_case $find_ignore_case
      set find_ignore_case "true"
      read_saif -auto_map_names -input $::env(ASIP_SAIF_FILE) -instance $::env(ASIP_SAIF_INSTANCE) -verbose
      set find_ignore_case $ORG_find_ignore_case
    }
    report_activity -driver > ${REPORTS_DIR}/${DESIGN_NAME}.mapped.activity.driver.start.rpt
######### END CHANGED FOR ASIP SCRIPTS ############
...
if {[info exists env(ASIP_SETUP)] && [file exists [which $::env(ASIP_SETUP)/custom_compile.tcl]]} {
  ...
  compile_ultra -spg -gate_clock -no_autoungroup
}
######### END CHANGED FOR ASIP SCRIPTS ############
...
report_resources -hierarchy > ${REPORTS_DIR}/${DESIGN_NAME}.mapped.resources.rpt
######### END CHANGED FOR ASIP SCRIPTS ############
report_clock_gating -nosplit > ${REPORTS_DIR}/${DCRM_FINAL_CLOCK_GATING_REPORT}
}
