...
set mode_constraints($mode1)            "m_func.tcl" ;# for mode1 specific SDC constraints

## Corner constraints; expand it as needed
set corner1 				"ss" ;# name of corner1	
set corner_constraints($corner1)        "c_ss.tcl" ;# for corner1 specific SDC constraints

set corner2 				"ff" ;# name of corner2	
set corner_constraints($corner2)        "c_ff.tcl" ;# for corner2 specific SDC constraints
...
set scenario_constraints($scenario1)    "s_func.ss.tcl" ;# for scenario1 specific SDC constraints

set scenario2 				"${mode1}::${corner2}" ;# scenario1 with mode1 and corner2
set scenario_constraints($scenario2)    "s_func.ff.tcl" ;# for scenario2 specific SDC constraints
...
foreach s [array name scenario_constraints] {
	...
	rm_source -file $scenario_constraints($s)
		if {[info exists ::env(ASIP_EXTRA_CONSTRAINTS_FILE)] &&
	    [file exists $::env(ASIP_EXTRA_CONSTRAINTS_FILE)]} {
			rm_source -file $::env(ASIP_EXTRA_CONSTRAINTS_FILE)
	}
}
