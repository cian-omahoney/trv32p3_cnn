...
#	if {[regexp "Error" $x]} {
# Changed to:
	if {[sizeof_collection [get_routing_rules -quiet $CTS_NDR_RULE_NAME]] == 0} {
######### END CHANGED FOR ASIP SCRIPTS ############
...
#	if {[regexp "Error" $x]} {
# Changed to:
	if {[sizeof_collection [get_routing_rules -quiet $CTS_INTERNAL_NDR_RULE_NAME]] == 0} {
######### END CHANGED FOR ASIP SCRIPTS ############
		## Create the NDR if it is not present
...
#	if {[regexp "Error" $x]} {
# Changed to:
	if {[sizeof_collection [get_routing_rules -quiet $CTS_LEAF_NDR_RULE_NAME]] == 0} {
######### END CHANGED FOR ASIP SCRIPTS ############
		## Create the NDR if it is not present
