...
# Please note that design_setup.tcl is the script sharing between FC and RTL-A RM. For RTL-A related variables please configure in rtl_setup.tcl.
# Please filled up 1,2,3,4 for design setup in RTL-A, and other variables might be not relevent to RTL-A.
# Please fill up the "Variables for general optimization use" section for SAIF and DFT.
...
set ADDITIONAL_SEARCH_PATH "${ASIP_SCRIPTS} ${ASIP_SCRIPTS}/mcmm ${ASIP_SCRIPTS}/user_plugin_scripts"
set ADDITIONAL_RTL_SETUP    $::env(ADDITIONAL_RTL_SETUP)  ;# Plug-in for ASIP-flows to overwritte any default configuration
set RTL_ID					$::env(RTL_ID)
...
## ASIP-DESIGN_SPECIFIC ##
set LIBRARY_SUFFIX		"${RTL_ID}.nlib" ;# Optional; suffix for the design library name ; default is unspecified  
...
set UPF_FILE 			""	;# A UPF file
					;# 	for DP: required
					;# 	for PNR: required if INIT_DESIGN_INPUT is ASCII in fc_pnr_setup.tcl; not required for DC_ASCII or DP_RM_NDM
...
set TCL_MCMM_SETUP_FILE		"mcmm.tcl"	;# Specify a Tcl script to create your corners, modes, scenarios and load respective constraints;
					;# two examples are provided as template.  
					...
					;# 	for PNR: required if INIT_DESIGN_INPUT is ASCII in icc2_pnr_setup.tcl; not required for DC_ASCII or DP_RM_NDM
## ASIP-LIB_SPECIFIC ## set TCL_PARASITIC_SETUP_FILE	""	;# Specify a Tcl script to read in your TLU+ files by using the read_parasitic_tech command;
					;# refer to the example in init_design.read_parasitic_tech_example.tcl
...
set DEF_FLOORPLAN_FILES			"" ;# Optional; DEF files which contain the floorplan information; not required for DP;
...
set SAIF_FILE_LIST			$::env(ASIP_SAIF_FILE) ;# Specify a SAIF file or a list of SAIF files and options for accurate power computation
					...
					;# Please specify single SAIF file, RTLA does not support multiple SAIF files as FC.
## ASIP-DESIGN_SPECIFIC ##
set SAIF_FILE_POWER_SCENARIO		"func::ss" ;# SAIF_FILE related; specify a power scenario where the SAIF is to be applied
## ASIP-DESIGN_SPECIFIC ##
set SAIF_FILE_SOURCE_INSTANCE		$::env(ASIP_SAIF_INSTANCE) ;# SAIF_FILE related; name of the instance of the current design as it appears in SAIF file.
...
set TCL_ANTENNA_RULE_FILE		"" ;# Antenna rule file
...
##########################################################################################
set TCL_USER_READ_RTL_PRE_SCRIPT 	"" ;# An optional Tcl file for init_design.from_rtl.tcl to be sourced before reading RTL
set TCL_USER_READ_RTL_POST_SCRIPT 	"" ;# An optional Tcl file for init_design.from_rtl.tcl to be sourced after reading RTL

set TCL_USER_COMPILE_PRE_SCRIPT 	"" ;# An optional Tcl file for compile.tcl to be sourced before compile
set TCL_USER_COMPILE_PRE_UNIFIED_SCRIPT "" ;# An optional Tcl file for compile.tcl to be sourced in the UNIFIED_FLOW before compile_fusion -from final_place
set TCL_USER_COMPILE_POST_SCRIPT 	"" ;# An optional Tcl file for compile.tcl to be sourced after compile
set TCL_USER_NON_PERSISTENT_SCRIPT 	"" ;# An optional Tcl file to be sourced in each step after open_block but prior to set_qor_strategy/set_stage commands.
...
set WRITE_GDS_PROPAGATE_PIN_MASK_TO_VIA_METAL true ;# Set true to propagate the mask property of metal1 pin to the corresponding
...
set WRITE_OASIS_PROPAGATE_PIN_MASK_TO_VIA_METAL true ;# Set true to propagate the mask property of metal1 pin to the corresponding
...
set ECO_OPT_RECIPE_FILE			"" ;# Optional; specify a metric type recipe for one or more eco_opt runs.  
					;#An example file is located in rm_fc_scripts/templates/eco_opt.recipe_example.tcl.
...
set ECO_OPT_EXTRACTION_MODE		"fusion_adv" ;# fusion_adv|in_design|none; default is fusion_adv; sets extract.starrc_mode to corresponding value;
					;# The fusion_adv and in_design modes require ECO_OPT_STARRC_CONFIG_FILE to be specified.
...
set TCL_USER_PSC_AUTO_DERIVE_MAPPING_RULE_FILE "" ;# A file for freeze silicon PSC (PSC, or gate array cell) auto derive mapping rule; 
					   ...
					   ;# an example is provided for the current node - refer to functional_eco.tcl.<node>.psc_auto_derive_mapping_rule_example 
					   ;# for sample commands 
...
# Fixing search_path for ASIP flows
if {[info exists env(ASIP_FLOW)]} {
lappend search_path ./rm_fc_scripts ./rm_setup $REDHAWK_SEARCH_PATH
} else {
set search_path [list ./rm_fc_scripts ./rm_setup $REDHAWK_SEARCH_PATH]
}
