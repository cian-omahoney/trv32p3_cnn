...
if {$MSCTS_TOPOLOGY == "htree"} {
	...
	if {$MSCTS_NET != ""} {
		set load_cells [get_cells -of [get_flat_pins -of [get_net $MSCTS_NET] -filter "direction==in"]]
		set_dont_touch $load_cells false
		set_dont_touch [get_lib_cells -of $load_cells] false
		set_size_only $load_cells false
		set_placement_status placed $load_cells
		set net_driver [get_flat_cell -of [get_flat_pins -filter "direction==out" -of $MSCTS_NET]]
		...
		set load_cells [get_cells -of [get_flat_pins -of [get_net -of $MSCTS_SOURCE] -filter "direction==in"]]
		set_dont_touch $load_cells false
		set_dont_touch [get_lib_cells -of $load_cells] false
		set_size_only $load_cells false
		set_placement_status placed $load_cells
	...
	foreach clock $MSCTS_CLOCK {
		set gm [resize_polygons -object [create_geo_mask -object [get_cells -phy -f design_type==macro]] -size {10}]
		#set gm2 [compute_polygons -operation and -objects1 $gm -objects2 $bbox]
	...
	update_timing	
	foreach clock $MSCTS_CLOCK {
		set tap_drivers [get_pins -of  [get_cells -physical *htree_r*c*] -filter "direction==out && related_clock==$clock"]
		set_multisource_clock_tap_options -clock $clock -num_taps [sizeof $tap_drivers] -driver_objects $tap_drivers
	}
	...
	set load_cells [get_cells -of [get_flat_pins -of [get_net $MSCTS_MESH_NET] -filter "direction==in"]]
	set_dont_touch $load_cells false
	set_dont_touch [get_lib_cells -of $load_cells] false
	set_size_only $load_cells false
        set_placement_status placed $load_cells
	...
	## Note : The following executes only if TCL_USER_MESH_ANNOTATION_SCRIPT is not sourced
		...
		foreach_in_collection scn [get_scenarios] {
			...
			foreach_in_collection itm [get_pins -phy -of [get_flat_cells mstap_*] -filter "direction==in && port_type==signal"] {
	...
	current_scenario $first_active
        ...
        update_timing
	set tap_drivers [get_pins -of  [get_cells -physical *mstap_r*c*] -filter "direction==out && related_clock==$MSCTS_CLOCK"]
	set_multisource_clock_tap_options -clock $MSCTS_CLOCK -driver_objects $tap_drivers -num_taps [sizeof $tap_drivers] 
