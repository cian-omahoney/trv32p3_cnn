...
if {[info exists env(ASIP_FLOW)]} {
	...
	redirect ${REPORTS_DIR}/${DESIGN_NAME}.resources.${DATE}.rpt        {report_resources -nosplit}
}
######### END CHANGED FOR ASIP SCRIPTS ############
...
######### BEGIN CHANGED FOR ASIP SCRIPTS ############
# Adding WRITE_QOR_DATA also in the flat-flow
if {[info exists env(ASIP_FLOW)] && $WRITE_QOR_DATA} {
	set QOR_DATA_LABEL rtl_opt
	# Just as done in the exploration flow using report_group 'placed' (includes congestion and power reports)
	write_qor_data -report_group placed -label $QOR_DATA_LABEL -report_list report_metrics -output $WRITE_QOR_DATA_DIR
	compare_qor_data -run_locations $WRITE_QOR_DATA_DIR -force -output $COMPARE_QOR_DATA_DIR/qorsum
}
######### END CHANGED FOR ASIP SCRIPTS ############
