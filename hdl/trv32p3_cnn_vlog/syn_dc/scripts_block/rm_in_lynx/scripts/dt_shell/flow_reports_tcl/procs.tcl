...
#            set rval(value,$app_option)                        "$app_option_value (N)"
      ...
      #needs special processing which is handled in this else statement
        ...
        #open curly bracket means the end of the value        
        if {[regexp {\{[ ]*$} $line]} {
          set inside_rail_lib_file_value 0
          set app_option_value "\{ $app_option_value \}"
	  continue
        }
        # add line to the value of rail.lib_file
        if {$inside_rail_lib_file_value} {
          lappend app_option_value [lindex $line 0]
	}

	#closed curly bracket at the beginning of the line means it is the very last part of the rail.lib_files line
        if {[regexp {^[ ]*\}} $line]} {
