...
module mem_DMb
  ( input                    ohe_selector_EX,
    ...
    input             [31:0] dm_addr2_dp_in, // addr
    ...
    input      signed [31:0] dmw_rd2_in, // w32
    input      signed [31:0] dmw_rd_in, // w32
    input      signed [31:0] dmw_wr_dp_in, // w32
    input                    hzd_stall_in,
    input                    kill_ID_in,
    output reg        [31:0] dm_addr2_out, // addr
    ...
    output reg               dmw_ld2_out, // std_logic
    output reg               dmw_ld_out, // std_logic
    output reg signed [31:0] dmw_rd2_dp_out, // w32
  ...
  begin : p_mem_DMb
    ...
    if (ohe_selector_EX) // (cp_extin_dmw_rd_ld_DMw_dm_addr_ID)
    begin
      // [ldst.n:64][cnn.n:105]
    ...
    case (bin_selector_EX)
      ...
      3'b101 : // (cp_extin_dmw_rd2_ld_DMw_dm_addr2_ID)
      begin
        // [cnn.n:125]
        dmw_rd2_dp_out = dmw_rd2_in;
      end
    ...
    if (ohe_selector_ID) // (mem_enab_dmw_rd2_ld_DMw_dm_addr2_ID, cp_extad_dmw_rd2_ld_DMw_dm_addr2_ID)
    begin
      // [cnn.n:125]
      if (!kill_ID_in && !hzd_stall_in)
        dmw_ld2_out = 1'b1;
      // [cnn.n:125]
      dm_addr2_out = dm_addr2_dp_in;
    end

    case (bin_selector_ID)
      3'b001 : // (mem_enab_dmw_rd_ld_DMw_dm_addr_ID, cp_extad_dmw_rd_ld_DMw_dm_addr_ID)
      begin
        // [ldst.n:64][cnn.n:105]
        if (!kill_ID_in && !hzd_stall_in)
          dmw_ld_out = 1'b1;
      ...
      3'b010 : // (mem_enab_DMb_st_dmb_wr_dm_addr_ID, cp_extad_DMb_st_dmb_wr_dm_addr_ID)
      begin
        // [ldst.n:99]
        if (!kill_ID_in && !hzd_stall_in)
          dmb_st_out = 1'b1;
      ...
      3'b011 : // (mem_enab_DMh_st_dmh_wr_dm_addr_ID, cp_extad_DMh_st_dmh_wr_dm_addr_ID)
      begin
        // [ldst.n:100]
        if (!kill_ID_in && !hzd_stall_in)
          dmh_st_out = 1'b1;
      ...
      3'b100 : // (mem_enab_DMw_st_dmw_wr_dm_addr_ID, cp_extad_DMw_st_dmw_wr_dm_addr_ID)
      begin
        // [ldst.n:101]
        if (!kill_ID_in && !hzd_stall_in)
          dmw_st_out = 1'b1;
      ...
      3'b101 : // (mem_enab_dmb_rd_ld_DMb_dm_addr_ID, cp_extad_dmb_rd_ld_DMb_dm_addr_ID)
      begin
        // [ldst.n:62]
        if (!kill_ID_in && !hzd_stall_in)
          dmb_ld_out = 1'b1;
      ...
      3'b110 : // (mem_enab_dmh_rd_ld_DMh_dm_addr_ID, cp_extad_dmh_rd_ld_DMh_dm_addr_ID)
      begin
        // [ldst.n:63]
        if (!kill_ID_in && !hzd_stall_in)
          dmh_ld_out = 1'b1;
