...
proc BuildSelectWidget::buttonRun {} {
  ...
  if {($gvar(reportType)== "DesignSummaryComparison") || ($gvar(reportType)== "ResourceSummaryComparison") || ($gvar(reportType)== "AppOptionComparison")} {
  ...
  set scriptToUse(AppOptionSummary)          $script_dir/gen_app_option_summary.$script_suffix
  set scriptToUse(AppOptionComparison)       $script_dir/gen_app_option_compare.$script_suffix
...
proc BuildSelectWidget::updateBuildReports {} {
  ...
  for { set row 0 } { $row < $rowCount } { incr row } {
    ...
    switch $gvar(reportType) {
      ...
      AppOptionSummary -
      AppOptionComparison {
        set reportFiles [concat $reportFiles [glob -nocomplain $buildDir/*/*/rpts/*/*.report_app_options]]
      }
    }
